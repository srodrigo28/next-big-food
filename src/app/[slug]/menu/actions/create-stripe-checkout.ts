...
export const createStripeCheckout = async ({
  ...
  if (tableCode) {
    searchParams.set("table", tableCode.trim().toUpperCase());
  }

  if (!process.env.STRIPE_SECRET_KEY) {
    return {
      checkoutUrl: `/${slug}/orders?${searchParams.toString()}`,
      sessionId: null,
    };
  }
  ...
  return {
    ...
    checkoutUrl: session.url ?? `/${slug}/orders?${searchParams.toString()}`,
  };
};
